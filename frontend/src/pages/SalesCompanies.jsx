...
const SalesCompanies = () => {
  ...
  const fetchCompanies = async () => {
    setLoading(true);
    try {
      const { data } = await api.get('/api/sales-companies');
      setCompanies(data);
      ...
      if (ledgerId) {
        navigate(`/ledger/sales/${ledgerId}`);
      }
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to load sales companies.');
    }
    setLoading(false);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (modalType === 'add') {
        await api.post('/api/sales-companies', formData);
      } else {
        await api.put(`/api/sales-companies/${selectedCompanyId}`, formData);
      }
      setShowModal(false);
      fetchCompanies();
    } catch (err) {
  ...
  const handleDelete = async (id) => {
    if (window.confirm('Are you sure you want to delete this customer? All linked sales must be deleted first.')) {
      try {
        await api.delete(`/api/sales-companies/${id}`);
        fetchCompanies();
      } catch (err) {
        alert(err.response?.data?.message || 'Failed to delete customer. Ensure no sales/payments are linked.');
      }
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-center">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-wide">Sales Companies</h1>
          <p className="text-sm text-slate-400 mt-1">Manage retail/contract customers, sales invoices, and receivables.</p>
        </div>
        <button
      ...
      <div className="glass rounded-3xl overflow-hidden">
        {loading ? (
          ...
          <div className="text-center py-12 text-slate-500">
            No sales companies found. Add a customer to begin.
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-800 bg-slate-900/20 text-xs font-semibold uppercase tracking-wider text-slate-400">
                  ...
                  <th className="py-4 px-6 text-right">Total Received</th>
                  <th className="py-4 px-6 text-right">Outstanding Receivables</th>
                ...
                {filteredCompanies.map((company) => (
                  <tr key={company._id} className="hover:bg-slate-900/20 transition-colors">
                    ...
                    <td className="py-4 px-6 text-right font-semibold text-slate-200">
                      {formatCurrency(company.totals.salesAmount)}
                    </td>
                    <td className="py-4 px-6 text-right font-semibold text-green-400">
                      {formatCurrency(company.totals.receivedAmount)}
                    </td>
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center justify-center gap-3">
                        <button
                          onClick={() => navigate(`/ledger/sales/${company._id}`)}
                          title="Open Ledger"
                          className="flex h-9 w-9 items-center justify-center rounded-lg bg-green-500/10 text-green-400 hover:bg-green-500 hover:text-white transition-all"
                        >
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/80 backdrop-blur-sm">
          <div className="w-full max-w-lg glass rounded-3xl p-6 relative shadow-2xl border border-slate-800 animate-scale-up">
            ...
            <h2 className="text-xl font-bold text-white tracking-wide mb-6">
              {modalType === 'add' ? 'Add Customer Company' : 'Edit Customer Details'}
            </h2>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                <textarea
                  ...
                  onChange={handleInputChange}
                  placeholder="Billing instructions, custom ledger notes..."
                  className="w-full rounded-xl border border-slate-800 bg-slate-900/60 py-3 px-4 text-sm text-white placeholder-slate-500 outline-none focus:border-green-500 transition-all resize-none"
                ></textarea>
              </div>
